...
public class Knob extends View {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        centerX = width/2;
        centerY = height/2;
        movingPadCenterX = externalRadius*2 - movingPadRelativeRadius*knobRadius;
        movingPadCenterY = 0 + movingPadRelativeRadius*knobRadius;
    ...
    void paintMovingPad(Canvas canvas) {
        paint.setColor(movingPadColor);
        paint.setStyle(Paint.Style.FILL);
        canvas.drawCircle(movingPadCenterX, movingPadCenterY, movingPadRelativeRadius*knobRadius, paint);
    }
    ...
    private int movingPadColor = Color.RED;
    private float movingPadRelativeRadius = 0.2f;
    ...
    boolean moving = false;        // used for object move
    float xPosViewClick, yPosViewClick, xPosClickRaw, yPosClickRaw;        // used for object move
    ...
    private Runnable userRunnableReleased = null;
    private Runnable userRunnablePushed = null;
    private Timer timer;
    // initialize
    ...
    private Paint paint;
    private Context ctx;
    private float externalRadius, knobRadius, centerX, centerY, movingPadCenterX, movingPadCenterY;
    ...
    void initListeners() {
        ...
        this.setOnTouchListener(new OnTouchListener() {
            ...
            public boolean onTouch(View view, MotionEvent motionEvent) {
                ...
                //timer.schedule(new opacityTask(),
                //        5000,        //initial delay
                //        1);  //subsequent rate

                //moving pad
                //canvas.drawCircle(centerX+knobRadius, centerY-knobRadius, movingPadRelativeRadius*knobRadius, paint);
                float x_moving_pad = (float) motionEvent.getX()-movingPadCenterX;
                float y_moving_pad = (float) motionEvent.getY()-movingPadCenterY;
                if (action == MotionEvent.ACTION_DOWN) {
                    if ((x_moving_pad*x_moving_pad + y_moving_pad*y_moving_pad)<movingPadRelativeRadius*knobRadius*movingPadRelativeRadius*knobRadius){
                        movingPadColor = Color.GREEN;
                        moving = true;
                        xPosViewClick = view.getLeft();
                        yPosViewClick =  view.getTop();
                        xPosClickRaw = motionEvent.getRawX();
                        yPosClickRaw =  motionEvent.getRawY();
                        takeEffect(true);
                ...
                if (moving) {
                    if (action == MotionEvent.ACTION_MOVE) {
                        float deltaX = motionEvent.getRawX() - xPosClickRaw;
                        float deltaY = motionEvent.getRawY() - yPosClickRaw;
                        int newX = (int) (xPosViewClick + deltaX);
                        int newY = (int) (yPosViewClick + deltaY);
                        if (newX >= 0 && (newX + centerX*2<=getScreenWidth())){
                            view.setLeft(newX);
                        }
                        if (newY >= 0 && (newY + centerY*2<=getScreenHeight())){
                            view.setTop(newY);
                        }
                        takeEffect(true);
                        return true;
                    ...
                    if (action == MotionEvent.ACTION_UP ) {
                        movingPadColor = Color.RED;
                        moving = false;
                        takeEffect(true);
                        view.setAlpha((float) 0.5);
                        return true;
                    }
                    return false; //todo what is return statement??
                 }
                /////////////////////////////////////////////////////////////////////////////////
                if (swipeDirection == SWIPEDIRECTION_VERTICAL) {  // vertical
                    ...
                    float x = (float) motionEvent.getX()-centerX;
                    float y = (float) motionEvent.getY()-centerY;
                    if (action == MotionEvent.ACTION_DOWN) {
                        view.setAlpha((float) 1);
                        if ((x*x + y*y)>knobRadius*knobRadius*knobCenterRelativeRadius*knobCenterRelativeRadius){
                            swipeing = true;
                        }else{
                            swipeing = false;
                            runPushed();
    ...
    public void setUserReleased(Runnable userRunnable) {
        this.userRunnableReleased = userRunnable;
    }

    public void runReleased() {   // to be initialized with setUserBehaviour()
        if (userRunnableReleased == null) return;
        userRunnableReleased.run();
    }

    public void setUserPushed(Runnable userRunnable) {
        this.userRunnablePushed = userRunnable;
    }

    public void runPushed() {   // to be initialized with setUserBehaviour()
        if (userRunnablePushed == null) return;
        userRunnablePushed.run();
    }
}
